...
function HomeContent() {
  ...
  const [selectedType, setSelectedType] = useState(() => {
    ...
    if (filterToken) {
      ...
      if (typeId && (typeId === 'all' || machineTypes.some(t => t.id === typeId))) {
        return typeId;
      }
    }
    return 'all';
  ...
  // Derived state: check if selected type is valid
  const validatedSelectedType = selectedType !== 'all' && !machineTypes.some(t => t.id === selectedType) 
    ? 'all' 
    : selectedType;
  
  // Update URL if type became invalid
  useEffect(() => {
  ...
  const handleTypeChange = (typeId: string) => {
    ...
    if (typeId === 'all') {
      // Remove filter parameter for 'all'
      router.replace('/');
    } else {
  ...
  const filteredTypes = validatedSelectedType === 'all' 
    ? machineTypes 
  ...
  return (
    <div style={{ backgroundColor: 'hsl(var(--color-background))' }} className="min-h-screen">
      <Header selectedType={validatedSelectedType} onTypeChange={handleTypeChange} />
      
      <main className="container mx-auto px-4 py-8">
        {machineTypes.length === 0 ? (
          <div className="text-center py-12">
            <h2 className="text-2xl font-bold mb-4">No Machine Types Yet</h2>
            <p style={{ color: 'hsl(var(--color-muted-foreground))' }} className="mb-6">
          ...
          <div className="space-y-8">
            {filteredTypes.map((type) => {
              ...
              return (
                <div key={type.id} style={{ backgroundColor: 'hsl(var(--color-card))' }} className="rounded-lg p-6 shadow-elevation-card">
                  ...
                  {typeMachines.length === 0 ? (
                    <p style={{ color: 'hsl(var(--color-muted-foreground))' }} className="text-center py-8">
                      No machines of this type yet. Click &quot;Add {type.title}&quot; to create one.
                    </p>
                  ) : (
